...
public class OkGoActivity extends PermissionApplyActivity/*BasicActivity*/ {
    ...
    private InitListener mTtsInitListener = new InitListener() {
        ...
        public void onInit(int code) {
            Log.d(mTag, "InitListener init() code = " + code);
            if (code == ErrorCode.SUCCESS) {
                // 初始化成功，之后可以调用startSpeaking方法
                // 注：有的开发者在onCreate方法中创建完合成对象之后马上就调用startSpeaking进行合成，
                // 正确的做法是将onCreate中的startSpeaking调用移至这里
            } else {
                Log.d(mTag, "初始化失败,错误码：" + code);
            }
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (mTts == null) {
            //创建单例失败，与21001错误为同样原因，参考 http://bbs.xfyun.cn/forum.php?mod=viewthread&tid=9688
            Log.d(mTag, "-----init tts fail-----");
        } else {
            this.setParam();
        }
    }
    ...
    private SynthesizerListener mTtsListener = new SynthesizerListener() {
        ...
        public void onCompleted(SpeechError error) {
            if (error == null) {
                Log.d(mTag, "语音正常播放完成");
                mTts.startSpeaking("收到人民币88.99元", null);
            } else {
                Log.d(mTag, "语音异常播放完成：" + error.getPlainDescription(true));
            }
        }
    ...
    public void onClickGet(View view) {
        if (mTts != null) {
            mTts.startSpeaking("收到人民币1697.45元", mTtsListener);
        }
