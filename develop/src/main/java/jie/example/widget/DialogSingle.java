...
public class DialogSingle extends Dialog implements View.OnClickListener {
	private OnClickDialog mOnClickDialog;

	public DialogSingle(Context context, String title, List<String> content,
			String btnCancel) {
	...
	private final class ListContentAdapter extends
		...
		private int mCheckedIndex = 0;// 默认选中第一项
		...
		protected void setCheckedIndex(int checkedIndex) {
			...
			if (mCheckedIndex == checkedIndex) {// 两次点中同一个
				mCheckedFlag.put(checkedIndex, !isChecked);// 改变同一个条目的选中状态
			} else {
				isChecked = mCheckedFlag.get(mCheckedIndex);
				mCheckedFlag.put(mCheckedIndex, false);// 先把之前选中的置为false

				mCheckedFlag.put(checkedIndex, true);// 再把当前选中的置为true
			}
